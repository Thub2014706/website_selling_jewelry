...
const FormAddress = ({showProvince, showDistrict, showWard, showAddress, showPhone, showCheck, onSubmit}) => {
    const user = useSelector((state) => state.auth.login.currentUser);

    const provinces = subVn.getProvinces();
    const [province, setProvince] = useState(showProvince ? showProvince : '');
    const [nameProvince, setNameProvince] = useState('');
    const districts = subVn.getDistrictsByProvinceCode(province);
    const [district, setDistrict] = useState(showDistrict ? showDistrict : '');
    const [nameDistrict, setNameDistrict] = useState('');
    const wards = subVn.getWardsByDistrictCode(district);
    const [ward, setWard] = useState(showWard ? showWard : '');
    ...
    const [address, setAddress] = useState(showAddress ? showAddress : '');

    const handleAddress = (e) => {
        setAddress(e.target.value);
    };

    const [phone, setPhone] = useState(showPhone ? showPhone : '');
    ...
    const [check, setCheck] = useState(showCheck ? showCheck : false);
    ...
    const handlesubmit = async (e) => {
        e.preventDefault();
        await onSubmit(axiosJWT, data, user?.accessToken);
    };
